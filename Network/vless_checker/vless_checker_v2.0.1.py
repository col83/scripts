...
class VlessCheckerTUI:
    ...
    def download_config_list(self, url):
        """Загрузить список конфигураций с улучшенной обработкой ошибок"""
        max_retries = 3
        for attempt in range(max_retries):
            try:
                self.add_log(f"Downloading configurations from {url} (attempt {attempt+1}/{max_retries})...")
                response = requests.get(url, timeout=30, stream=True)
                response.raise_for_status()
                
                content = b""  # Используем байты для накопления
                for chunk in response.iter_content(chunk_size=4096):  # Уменьшенный размер чанка
                ...
                if not content:
                    self.add_log("Downloaded empty content", 'error')
                    return []
                ...
                for encoding in encodings:
                    try:
                        ...
                        self.add_log(f"Successfully decoded using {encoding}", 'success')
                        break
                    except UnicodeDecodeError:
                        continue
                
                if decoded_content is None:
                    self.add_log("Failed to decode content with any encoding", 'error')
                    return []
                ...
                self.add_log(f"Network error (attempt {attempt+1}): {e}", 'warning')
                if attempt < max_retries - 1:
                    time.sleep(2)  # Пауза перед повторной попыткой
                    continue
                self.add_log("Max retries reached, download failed", 'error')
                return []
            except Exception as e:
                self.add_log(f"Unexpected error (attempt {attempt+1}): {e}", 'error')
                if attempt < max_retries - 1:
                    time.sleep(2)
                    continue
                return []
